      ...
      <el-dialog :title="dialogTitle" :visible.sync="addDialogVisible" :close-on-click-modal="false">
        <el-form :disabled="isShow" :model="data" label-width="120px" :inline="true">
          <el-form-item label="场景ID">
            <el-input v-model="data.sid" :disabled="dialogTitle != '新增'" />
          </el-form-item>
          <el-form-item label="源数据ID">
            <el-select v-model="options.scIdOptions.value" filterable placeholder="请选择">
              <el-option
...
<script>
import { findCdDatasDistributionList, mySceneList, addScene, editScene, deleteScene, imgUpload, fileDataUpload, sceneFileDownLoad, getPreviewFileUrl, getSceneTypeTree, addSceneUser } from '@/api/simulation'
...
export default {
  ...
  methods: {
    ...
    loadData: function(pageNum, pageSize) {
      ...
      mySceneList(this.searchObj).then(response => {
    ...
    getCollectionDatas() {
      var that = this
      findCdDatasDistributionList({ pageNum: 1, pageSize: 10000 }).then(response => {
